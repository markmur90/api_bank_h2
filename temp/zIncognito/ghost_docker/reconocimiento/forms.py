...
class EjecucionReconForm(forms.Form):
    ...
    identificadores = forms.CharField(
        label="Identificadores (coma separados=usuario, ip, correo, token, session_id, admin, debug, clave)",
        required=True
    )
    ...
    repeticiones = forms.IntegerField(
        ...
        min_value=1,
        max_value=10,
        label="Repeticiones (Cuántas veces ejecutar max=10)"
    )

    delay = forms.IntegerField(
        initial=10,
        min_value=1,
        label="Delay entre repeticiones (segundos)"
    )

    tiempo_navegacion = forms.IntegerField(
        initial=60,
        min_value=60,
        label="Tiempo de navegación (segundos) min=(60=1m) max=(1800=30m)",
        help_text="Puede calcularse automáticamente si seleccionas la opción correspondiente."
    )
...
class UrlSeleccionForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        with open(os.path.join(settings.BASE_DIR, "ghost_config.json"), "r", encoding="utf-8") as f:
            data = json.load(f)
