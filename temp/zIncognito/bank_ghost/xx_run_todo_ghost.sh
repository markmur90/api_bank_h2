...
configure_nginx(){
  ...
  if sudo nginx -t &>>"$OPERATION_LOG"; then
    ...
    echo "ERROR: Prueba de configuración de Nginx fallida. Revisa $OPERATION_LOG" >&2
    exit 1
  fi
...
open_interface(){
  ...
  firefox --new-window "$APP_URL" &>>"$OPERATION_LOG"
  end_block "Abrir Interfaz"
}

log_startup(){
  # start_block "Registro de Inicio"
  echo "Despliegue completado en $(date)" | tee -a "$OPERATION_LOG"
  # end_block "Registro de Inicio"
}
