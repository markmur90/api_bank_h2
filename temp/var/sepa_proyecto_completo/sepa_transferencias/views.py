...
from .helpers import generate_deterministic_id, generate_uuid_id, obtener_ruta_log_transferencia, obtener_ruta_schema_transferencia
...
def crear_transferencia(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            transferencia.payment_id = generate_uuid_id()
...
class CrearBulkTransferView(View):
    ...
    def post(self, request):
        ...
        if all([bulk_form.is_valid(), group_form.is_valid(), payment_info_form.is_valid()]):
            ...
            bulk.payment_id = generate_uuid_id()
