...
def generar_xml_pain001(transferencia: Transfer, payment_id: str) -> str:
    ...
    registrar_log("OAUTH_LOGS", payment_id, tipo_log='XML', extra_info=f"XML pain.001 generado en {xml_path}")
    return xml_path
...
def generar_archivo_aml(transferencia: Transfer, payment_id: str) -> str:
    ...
    registrar_log("OAUTH_LOGS", payment_id, tipo_log='AML', extra_info=f"Archivo AML generado en {aml_path}")
    return aml_path
...
def generar_pdf_transferencia(transferencia: Transfer) -> str:
    ...
    if os.path.exists(qr_path):
        os.remove(qr_path)
    registrar_log("OAUTH_LOGS", transferencia.payment_id, tipo_log='TRANSFER', extra_info=f"PDF generado en {pdf_path}")
    return pdf_path
...
def send_transfer(transfer: Transfer, use_token: str = None, use_otp: str = None,
    ...
    registrar_log("OAUTH_LOGS", pid, headers_enviados=headers, request_body=body, tipo_log='TRANSFER', extra_info="Enviando transferencia SEPA")
    try:
        resp = requests.post(API_URL, headers=headers, json=body, timeout=TIMEOUT_REQUEST)
        registrar_log("OAUTH_LOGS", pid, tipo_log='TRANSFER', response_text=resp.text, extra_info="Respuesta del API SEPA")
        resp.raise_for_status()
    except requests.RequestException as e:
        err = str(e)
        registrar_log("OAUTH_LOGS", pid, error=err, tipo_log='ERROR', extra_info="Error HTTP enviando transferencia")
        raise
    data = resp.json()
    transfer.auth_id = data.get('authId')
    transfer.status = data.get('transactionStatus', transfer.status)
    transfer.save()
    registrar_log("OAUTH_LOGS", pid, tipo_log='TRANSFER', extra_info="Transferencia enviada con éxito")
    ...
    try:
        ...
        validar_xml_pain001(xml_path)
        validar_aml_con_xsd(aml_path)
        registrar_log("OAUTH_LOGS", pid, tipo_log='TRANSFER' ,extra_info="Validación XML/AML completada")
    except Exception as e:
        registrar_log("OAUTH_LOGS", pid, error=str(e), tipo_log='ERROR', extra_info="Error generando XML/AML posterior")
    return resp
...
def get_access_token(payment_id: str = None, force_refresh: bool = False) -> str:
    ...
    registrar_log("OAUTH_LOGS", tipo_log='AUTH', extra_info="Obteniendo Access Token (Client Credentials)")
    data = {'grant_type': 'client_credentials', 'scope': settings.SCOPE}
    headers = {'Content-Type': 'application/x-www-form-urlencoded'}
    registrar_log("OAUTH_LOGS", tipo_log='AUTH', headers_enviados=headers, request_body=data)
    try:
        ...
        err = str(e)
        registrar_log("OAUTH_LOGS", tipo_log='ERROR', error=err, extra_info="Error al obtener Access Token")
        raise
    token = resp.json().get('access_token')
    if not token:
        err = resp.json().get('error_description', 'Sin access_token en respuesta')
        registrar_log("OAUTH_LOGS", tipo_log='AUTH', error=err, extra_info="Token inválido recibido")
...
def get_access_token_jwt(payment_id: str, force_refresh: bool = False) -> str:
    ...
    registrar_log("OAUTH_LOGS", tipo_log='AUTH', extra_info="Obteniendo Access Token (JWT Assertion)")
    now = int(time.time())
    payload = {
    ...
    registrar_log("OAUTH_LOGS", tipo_log='AUTH', request_body=data)
    try:
        resp = requests.post(TOKEN_URL, data=data, timeout=TIMEOUT_REQUEST)
        registrar_log("OAUTH_LOGS", tipo_log='AUTH', response_text=resp.text)
        resp.raise_for_status()
    except Exception as e:
        err = str(e)
        registrar_log("OAUTH_LOGS", tipo_log='ERROR', error=err, extra_info="Error obteniendo Access Token JWT")
        raise
    token = resp.json().get('access_token')
    if not token:
        err = resp.json().get('error_description', 'Sin access_token en respuesta')
        registrar_log("OAUTH_LOGS", tipo_log='AUTH', error=err, extra_info="Token JWT inválido")
        raise Exception(f"Token JWT inválido: {err}")
    registrar_log("OAUTH_LOGS", tipo_log='AUTH', extra_info="Token JWT obtenido correctamente")
    return token
...
def update_sca_request(transfer: Transfer, action: str, otp: str, token: str) -> requests.Response:
    ...
    registrar_log("OAUTH_LOGS", tipo_log='SCA', headers_enviados=headers, request_body=payload, extra_info="Actualizando SCA")
    resp = requests.patch(url, headers=headers, json=payload, timeout=TIMEOUT_REQUEST)
    registrar_log("OAUTH_LOGS", tipo_log='SCA', response_text=resp.text, extra_info="Respuesta SCA")
    resp.raise_for_status()
    data = resp.json()
    transfer.auth_id = data.get('authId')
    transfer.status = data.get('transactionStatus', transfer.status)
...
def fetch_transfer_details(transfer: Transfer, token: str) -> dict:
    ...
    registrar_log("OAUTH_LOGS", tipo_log='TRANSFER', headers_enviados=headers, extra_info="Obteniendo estado de transferencia")
    resp = requests.get(url, headers=headers, timeout=TIMEOUT_REQUEST)
    registrar_log("OAUTH_LOGS", tipo_log='TRANSFER', response_text=resp.text, extra_info="Respuesta fetch status")
    resp.raise_for_status()
    data = resp.json()
    transfer.status = data.get('transactionStatus', transfer.status)
    transfer.save()
    ...
    registrar_log("OAUTH_LOGS", tipo_log='TRANSFER', extra_info="Pain002 generado y validado")
    return data
...
def build_auth_url(state, challenge, redirect_uri):
    try:
        ...
        registrar_log("OAUTH_LOGS", tipo_log="AUTH", extra_info="URL de autorización construida", request_body={"url": url})
        return url
    except Exception as e:
        registrar_log("OAUTH_LOGS", tipo_log="ERROR", error=str(e), extra_info="Error construyendo URL de autorización")
        raise
...
def crear_challenge_mtan(transfer: Transfer, token: str, payment_id: str) -> str:
    ...
    registrar_log("OAUTH_LOGS", payment_id, headers_enviados=headers, request_body=payload, extra_info="Iniciando MTAN challenge", tipo_log='OTP')
    resp = requests.post(AUTH_URL, headers=headers, json=payload, timeout=TIMEOUT_REQUEST)
    registrar_log("OAUTH_LOGS", payment_id, response_text=resp.text, tipo_log='OTP')
    resp.raise_for_status()
    return resp.json()['id']
...
def crear_challenge_phototan(transfer: Transfer, token: str, payment_id: str):
    ...
    registrar_log("OAUTH_LOGS", payment_id, headers_enviados=headers, request_body=payload, extra_info="Iniciando PhotoTAN challenge", tipo_log='OTP')
    resp = requests.post(AUTH_URL, headers=headers, json=payload, timeout=TIMEOUT_REQUEST)
    registrar_log("OAUTH_LOGS", payment_id, response_text=resp.text, tipo_log='OTP')
    resp.raise_for_status()
    data = resp.json()
...
def resolver_challenge(challenge_id: str, token: str, payment_id: str) -> str:
    ...
    while True:
        ...
        registrar_log("OAUTH_LOGS", payment_id, response_text=resp.text, extra_info=f"Comprobando estado challenge {challenge_id}", tipo_log='OTP')
        data = resp.json()
        status = data.get('status')
        if status == 'VALIDATED':
            otp = data.get('otp')
            registrar_log("OAUTH_LOGS", payment_id, extra_info=f"OTP validado: {otp}", tipo_log='OTP')
            return otp
        if status in ('EXPIRED', 'REJECTED', 'EIDP_ERROR'):
            msg = f"Challenge fallido: {status}"
            registrar_log("OAUTH_LOGS", payment_id, error=msg, tipo_log='ERROR')
            raise Exception(msg)
...
def crear_challenge_pushtan(transfer: Transfer, token: str, payment_id: str) -> str:
    ...
    if response.status_code != 201:
        error_msg = handle_error_response(response)
        registrar_log("OAUTH_LOGS", payment_id, headers, response.text, error=error_msg, tipo_log='ERROR')
...
def crear_challenge_autorizacion(transfer, token):
    pid = transfer.payment_id
    try:
        registrar_log("OAUTH_LOGS", pid, extra_info="Iniciando challenge OTP", tipo_log='OTP')
        payload = {
        ...
        registrar_log("OAUTH_LOGS", pid, headers_enviados=headers, request_body=payload, tipo_log='OTP')
        resp = requests.post(AUTH_URL, headers=headers, json=payload, timeout=TIMEOUT_REQUEST)
        registrar_log("OAUTH_LOGS", pid, response_text=resp.text, tipo_log='OTP')
        resp.raise_for_status()
        cid = resp.json().get('id')
        registrar_log("OAUTH_LOGS", pid, extra_info=f"Challenge creado con ID {cid}", tipo_log='OTP')
        return cid
    except Exception as e:
        registrar_log("OAUTH_LOGS", pid, error=str(e), extra_info="Error al crear challenge", tipo_log='ERROR')
        raise
...
def resolver_challenge_pushtan(challenge_id: str, token: str, payment_id: str) -> str:
    ...
    while True:
        ...
        if status == 'PENDING' and time.time() - start < 300:
            ...
            registrar_log("OAUTH_LOGS", payment_id, headers, error=msg, tipo_log='ERROR')
            raise TimeoutError(msg)
            time.sleep(1)
            continue
        elif status == "EXPIRED":
            msg = "El challenge ha expirado (status=EXPIRED)"
            registrar_log("OAUTH_LOGS", payment_id, headers, response.text, error=msg, tipo_log='ERROR')
            raise Exception(msg)
