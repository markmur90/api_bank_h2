...
PROYECTO=$(zenity --entry --title="Seleccionar proyecto 10" \
...
TASK_FILE="$PROY_DIR/tareas_10.txt"
CONFIG_FILE="$PROY_DIR/config_10.txt"
TIME_LOG="$PROY_DIR/tiempos_10.log"
...
mostrar_gestor() {
    ...
    mapfile -t tareas_array < <(awk -F'|' '{printf "%d|%s|%s|%s\n", NR, $1, $2, $3}' "$TASK_FILE")

    seleccion=$(zenity --list --title="Tareas – $PROYECTO" \
        --text="$encabezado" \
        --column="N°" --column="Descripción" --column="Estado" --column="Creación" \
        "${tareas_array[@]}" \
    ...
    fecha=$(awk -F'|' "NR==$numero{print \$3}" "$TASK_FILE")
    ...
    case $accion in
        Editar)
            ...
            [ -n "$nueva" ] && sed -i "${numero}s|.*|$nueva|$estado|$fecha|" "$TASK_FILE"
            ;;
        Eliminar)
        ...
        "Actualizar estado")
            if [[ "$estado" == pendiente ]]; then
                sed -i "${numero}s|$desc|$desc|completada|" "$TASK_FILE"
            else
                sed -i "${numero}s|$desc|$desc|pendiente|" "$TASK_FILE"
            fi
            ;;
    esac
...
activar_gestor() {
    ...
    while [ -f "$ACTIVE_FILE" ]; do
        nueva=$(zenity --entry --title="Nueva tarea (opcional)" --text="Descripción (dejar vacío para ver lista):")
        if [[ -n "$nueva" ]]; then
            fecha=$(date '+%Y-%m-%d')
