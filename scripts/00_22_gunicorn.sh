...
liberar_puertos() {
    for port in "${PUERTOS[@]}"; do
        if lsof -i :$port &>/dev/null; then
            echo -e "\033[1;34m🔌 Liberando puerto $port...\033[0m" | tee -a $LOG_SISTEMA
...
limpiar_y_salir() {
    echo -e "\n\033[1;33m🧹 Deteniendo todos los servicios...\033[0m" | tee -a $LOG_SISTEMA
nohup pkill -f "gunicorn" &>/dev/null || true > logs/gunicorn_default.log 2>&1 &
    ...
    liberar_puertos
    echo -e "\033[1;32m✅ Todos los servicios detenidos.\033[0m" | tee -a $LOG_SISTEMA
    echo -e "$LOGO_SEP\n" | tee -a $LOG_SISTEMA
    exit 0
}

iniciar_entorno() {
    echo -e "\033[1;36m📦 Activando entorno virtual y configuración...\033[0m" | tee -a $LOG_SISTEMA
    cd "$PROJECT_ROOT"
...
verificar_seguridad() {
    if [[ "${ENVIRONMENT:-local}" != "local" ]]; then
        echo -e "\033[1;31m🔒 Verificando conexión segura: VPN + Tor...\033[0m" | tee -a $LOG_SISTEMA
        if ! curl --socks5-hostname 127.0.0.1:9050 https://check.torproject.org | grep -q "Congratulations"; then
            echo -e "\033[1;31m❌ Error: No estás conectado por Tor. Abortando por seguridad.\033[0m" | tee -a $LOG_SISTEMA
            exit 1
        fi
        echo -e "\033[1;32m✅ Tor activo. Entorno seguro.\033[0m" | tee -a $LOG_SISTEMA
    fi
}

# === EJECUCIÓN PRINCIPAL ===

echo -e "\033[7;30m🚀 Iniciando Gunicorn, honeypot y livereload...\033[0m" | tee -a $LOG_SISTEMA
...
nohup bash "${SCRIPTS_DIR}/configurar_gunicorn.sh" > logs/gunicorn_default.log 2>&1 &
    ...
    echo -e "\033[1;31m❌ Error al configurar Gunicorn. Consulta $STARTUP_LOG\033[0m" | tee -a $LOG_SISTEMA
    exit 1
}

echo -e "\033[1;34m🌀 Lanzando servicios secundarios...\033[0m" | tee -a $LOG_SISTEMA
...
echo -e "\033[7;30m🚧 Servicios activos. Ctrl+C para detener.\033[0m" | tee -a $LOG_SISTEMA
echo -e "$LOGO_SEP\n" | tee -a $LOG_SISTEMA
