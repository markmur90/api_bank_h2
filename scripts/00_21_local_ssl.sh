...
if [[ ! -f "$CERT_CRT" || ! -f "$CERT_KEY" ]]; then
    echo "⚠️ Certificados no encontrados. Generando nuevos..." | tee -a $LOG_SISTEMA
    mkdir -p certs
...
if sudo lsof -i :8443 | grep -q LISTEN; then
    echo "🧅 Puerto 8443 ya está en uso (probablemente Nginx)." | tee -a $LOG_SISTEMA
    
    if sudo lsof -i :8000 | grep -q LISTEN; then
        echo "⚠️ Puerto 8000 en uso. Liberando..." | tee -a $LOG_SISTEMA
        sudo fuser -k 8000/tcp
        sleep 2
    fi

    echo "🚀 Ejecutando Gunicorn como backend en http://0.0.0.0:8000" | tee -a $LOG_SISTEMA
nohup gunicorn config.wsgi:application --bind 0.0.0.0:8000 > logs/gunicorn_ssl.log 2>&1 &
else
    echo "🌐 Levantando entorno local con Gunicorn + SSL en https://0.0.0.0:8443" | tee -a $LOG_SISTEMA
    echo "🔐 Certificado: $CERT_CRT" | tee -a $LOG_SISTEMA
