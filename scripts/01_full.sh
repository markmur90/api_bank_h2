...
LOG_DIR_BASE="$PROJECT_ROOT/scripts/logs"
LOG_SISTEMA="$LOG_DIR_BASE/sistema/sistema.log"
LOG_BACKUP="$LOG_DIR_BASE/backup/backup_$(date +%Y%m%d_%H%M).log"
LOG_DEPLOY="$LOG_DIR_BASE/despliegue/deploy_$(date +%Y%m%d_%H%M).log"
mkdir -p "$(dirname "$LOG_SISTEMA")" "$(dirname "$LOG_BACKUP")" "$(dirname "$LOG_DEPLOY")"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -H|--do-heroku)       DO_HEROKU=true ;; | tee -a "$LOG_DEPLOY"
        -u|--do-varher)       DO_VARHER=true ;;
        -G|--do-gunicorn)     DO_GUNICORN=true ;; | tee -a "$LOG_DEPLOY"
...
ejecutar_si_activo "DO_ZIP_SQL" "Crear zip y sql" "bash $SCRIPTS_DIR/00_02_zip_backup.sh" | tee -a "$LOG_BACKUP"
echo ""
echo ""
echo ""
sleep 1
...
ejecutar_si_activo "DO_HEROKU" "Subir el proyecto a la web" "bash $SCRIPTS_DIR/00_16_subir_heroku.sh" | tee -a "$LOG_DEPLOY"
echo ""
echo ""
echo ""
sleep 1
...
if [[ "$DO_LOCAL_SSL" == true && "$DO_GUNICORN" == true ]]; then
    echo -e "\033[1;31m❌ No puedes ejecutar DO_LOCAL_SSL y DO_GUNICORN al mismo tiempo.\033[0m" | tee -a "$LOG_DEPLOY"
    exit 1
fi
...
ejecutar_si_activo "DO_GUNICORN" "Iniciar Gunicorn, honeypot y livereload" "bash $SCRIPTS_DIR/00_22_gunicorn.sh" | tee -a "$LOG_DEPLOY"
echo ""
echo ""
echo ""
sleep 1
